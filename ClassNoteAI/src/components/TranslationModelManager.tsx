...
const TRANSLATION_MODELS: TranslationModelInfo[] = [
  {
    ...
    displayName: 'M2M100 (多語言翻譯, int8 量化)',
    size: '~440MB',
    description: '支持100+種語言互譯，高品質翻譯，使用CTranslate2引擎',
  },
];
...
export default function TranslationModelManager() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // 使用統一路徑 (downloads module 會自動使用 app data dir)
      ...
      const unlisten = await listen<{
        ...
        percent: number;
        speed_mbps: number;
      }>('translation_download_progress', (event) => {
        const { percent, speed_mbps } = event.payload;
        setDownloadProgress(Math.round(percent));
        setStatusMessage(`下載中... ${percent.toFixed(1)}% (${speed_mbps.toFixed(1)} MB/s)`);
      });

      try {
        await downloadTranslationModel(selectedModel, modelsDir, (progress) => {
          // Fallback in case event not received
    ...
    <div className="mb-6 bg-white dark:bg-slate-800 rounded-lg shadow p-6">
      ...
      <div className="space-y-4">
        ...
        <div className="text-xs text-gray-500 dark:text-gray-400 space-y-1 pt-2 border-t border-gray-200 dark:border-gray-700">
          <p>• 首次使用需要下載或準備模型文件</p>
          <p>• M2M100 模型支持100+種語言互譯（推薦）</p>
          <p>• 模型會自動下載到應用數據目錄的 models/ 下</p>
