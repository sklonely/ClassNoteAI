  ...
  const loadLectureData = async (id: string) => {
    try {
      ...
      if (lecture) {
        ...
        if (lecture.course_id) {
          ...
          if (course) {
            ...
            if (course.syllabus_info) {
              const { topic, schedule } = course.syllabus_info;
              const parts = [];
              if (topic) parts.push(`Topic: ${topic}`);
              if (schedule && schedule.length > 0) parts.push(`Schedule: ${schedule.join(', ')}`);
        ...
        // Set initial prompt with rich context
        // Format: "Context info. Glossary: keywords."
