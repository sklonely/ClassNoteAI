use classnoteai_lib::translation::ctranslate2::CT2Translator;
use std::path::PathBuf;

fn translation_model_dir_from_env() -> Option<PathBuf> {
    std::env::var_os("CLASSNOTEAI_TRANSLATION_MODEL_DIR").map(PathBuf::from)
}

fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", "=".repeat(80));
    println!("Rust CT2 Translation Example");
    println!("{}", "=".repeat(80));

    let Some(model_dir) = translation_model_dir_from_env() else {
        println!("Set CLASSNOTEAI_TRANSLATION_MODEL_DIR to run this example.");
        return Ok(());
    };

    println!("\n[Rust] Model directory: {:?}", model_dir);

    let test_texts = vec![
        "Hello".to_string(),
        "Hello world".to_string(),
        "Hello, how are you?".to_string(),
        "Good morning".to_string(),
        "Thank you".to_string(),
    ];

    println!("\n[Rust] Loading translation model...");
    let mut translator = CT2Translator::new();
    translator.load_model(model_dir.to_string_lossy().as_ref())?;

    println!("\n{}", "=".repeat(80));
    println!("Starting translation checks");
    println!("{}", "=".repeat(80));

    let results = translator.translate_batch(&test_texts, Some("zh"), Some("en"))?;

    for (i, (text, translated)) in test_texts.iter().zip(results.iter()).enumerate() {
        println!("\n{}", "=".repeat(80));
        println!("Case {}: '{}'", i + 1, text);
        println!("{}", "=".repeat(80));
        println!("[Rust] Translation result: '{}'", translated);
    }

    Ok(())
}
