...
async fn test_opus_mt_fixes() {
    ...
    for (idx, text) in CRITICAL_TEST_CASES.iter().enumerate() {
        ...
        match model.translate(text, "en", "zh").await {
            Ok(translated) => {
                ...
                println!(
                    "     包含中文: {}, 為空: {}, 重複: {}",
                    has_chinese, is_empty, is_repetitive
                );
            }
            Err(e) => {
                eprintln!("  ❌ 翻譯失敗: {}", e);
    ...
    println!(
        ...
        total_success,
        total_count,
        total_success as f32 / total_count as f32 * 100.0
    );
    println!(
        "  包含中文: {}/{} ({:.1}%)",
        total_chinese,
        total_count,
        total_chinese as f32 / total_count as f32 * 100.0
    );
    println!(
        "  空輸出: {}/{} ({:.1}%)",
        total_empty,
        total_count,
        total_empty as f32 / total_count as f32 * 100.0
    );
    println!(
        "  重複輸出: {}/{} ({:.1}%)",
        total_repetitive,
        total_count,
        total_repetitive as f32 / total_count as f32 * 100.0
    );
