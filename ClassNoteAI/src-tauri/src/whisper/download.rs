    ...
    let mut file = if config.output_path.exists() {
        ...
        if let Some(expected_size) = config.expected_size {
            if existing_size == expected_size {
                ...
                // 文件存在但不完整，支持斷點續傳
                println!(
                    "[下載] 發現不完整文件 ({:.2} MB)，繼續下載...",
                    existing_size as f64 / 1_000_000.0
                );
